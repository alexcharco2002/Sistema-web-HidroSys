...
class NotificacionBulkCreate(BaseModel):
    """Schema para crear una notificacion para varios usuarios en una sola llamada."""
    ids_usuarios_sistema: Optional[List[int]] = None
    titulo: str
    mensaje: str
    tipo: Optional[str] = "info"
    prioridad: Optional[str] = "media"

    @field_validator('ids_usuarios_sistema')
    @classmethod
    def validate_ids(cls, v):
        if v is None:
            return v
        ids = sorted({int(item) for item in v if item})
        if not ids:
            raise ValueError('Debes seleccionar al menos un usuario')
        if len(ids) > 1000:
            raise ValueError('No puedes enviar mas de 1000 destinatarios por solicitud')
        return ids
    ...
    def validate_tipo(cls, v):
        tipos_validos = ['info', 'alerta', 'error', 'sistema', 'exito', 'advertencia', 'mantenimiento']
        if v not in tipos_validos:
            raise ValueError(f"Tipo invalido. Debe ser uno de: {', '.join(tipos_validos)}")
        return v
    ...
    def validate_prioridad(cls, v):
        prioridades_validas = ['baja', 'media', 'alta', 'critica']
        if v not in prioridades_validas:
            raise ValueError(f"Prioridad invalida. Debe ser: {', '.join(prioridades_validas)}")
        return v
