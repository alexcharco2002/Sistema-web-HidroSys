...
class ConfiguracionMora(Base):
    ...
    tipo_periodo = Column(
        String(20),
        CheckConstraint("tipo_periodo IN ('dias', 'meses')"),
        default='dias',
        nullable=False
    )
    ...
    # Meses de gracia
    meses_gracia = Column(Integer, default=0, nullable=True)
    ...
    def __repr__(self):
        #  para mostrar tipo_periodo
        periodo_info = f"{self.dias_gracia}d" if self.tipo_periodo == 'dias' else f"{self.meses_gracia}m"
        return f"<ConfiguracionMora(id={self.id_configuracion_mora}, nombre='{self.nombre}', periodo={periodo_info}, activo={self.activo})>"
